#!/bin/bash
# ============================================================
#   MZEE KOBE STORE - SOCKS Management
# ============================================================
...
show_header() {
    ...
    echo -e "${MAGENTA}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo -e "${RED}▌  ★  SOCKS MENU  ★   Mzee Kobe Store                         ▐${NC}"
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
...
check_xray_users() {
    echo ""
    echo -e "${RED}▌  LOGGED IN SOCKS USERS                                       ▐${NC}"
    echo -e "${MAGENTA}╔═══════════════════╦════════════════════╦═══════════════════╗${NC}"
...
check_configs() {
    echo ""
    echo -e "${RED}▌  SOCKS CONFIG FILES                                          ▐${NC}"
...
check_bandwidth() {
    echo ""
    echo -e "${RED}▌  BANDWIDTH USED - SOCKS                                      ▐${NC}"
    local LOG="/var/log/xray/access.log"
...
while true; do
    show_header
    echo ""
    echo -e "  ${YELLOW}[1]${NC}.Add SOCKS Account      ${YELLOW}[2]${NC}.Check Logged In Users"
    echo -e "  ${YELLOW}[3]${NC}.Delete SOCKS Account   ${YELLOW}[4]${NC}.Block Sites"
    echo -e "  ${YELLOW}[5]${NC}.Renew SOCKS Account    ${YELLOW}[6]${NC}.Check Configs Files"
    ...
    case "$opt" in
        1)
            ...
            if [ -z "$user" ] || [ -z "$days" ]; then
                ...
                echo -e "  ${BGREEN}[OK]${NC} SOCKS account created: $user ($days days)"
            fi
            echo ""; read -rp "  Press Enter to continue..."
            ;;
        2) check_xray_users ;;
